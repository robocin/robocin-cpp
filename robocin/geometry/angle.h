...
class Angle {
 public:
  ...
  template <angle_internal::other_floating_point<type> G>
  constexpr Angle(Angle<G>&& other) noexcept : value_{static_cast<type>(other)} {}
  ...
  [[nodiscard]] constexpr Angle diffTo(type other) && {
    return value_ = smallestAngleDiff(value_, other), std::move(*this);
  }
  [[nodiscard]] constexpr Angle diffTo(type other) const& { return Angle{*this}.diffTo(other); }

  [[nodiscard]] constexpr Angle absDiffTo(type other) && {
    return value_ = absSmallestAngleDiff(value_, other), std::move(*this);
  }
  [[nodiscard]] constexpr Angle absDiffTo(type other) const& {
    return Angle{*this}.absDiffTo(other);
  }
